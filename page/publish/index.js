...
document.querySelector('.send').addEventListener('click', async e => {
     ...
     */
    // 4.2 发布文章页面接收参数判断（共用同一套表单)
    // 为了保证我定义的变量，不影响全局作用域，使用一个自调用的函数
    // 对参数进行接收，打印，并且进行确认
    ; (function () {
        // location.search 是查询参数字符串
        ...
        params.forEach(async (value, key) => {
            ...
            // 证明当前有要编辑的 文章 id 被传入过来
            if (key === 'id') {
                // 4.3 修改标题和按钮文字
                // 如果 key 为 id 的话，就说明这里需要进行修改而不是发布，所以要修改一下一些标签上的文字内容
                ...
                Object.keys(dataObj).forEach(key => {
                    if (key === 'rounded') {
                        // 封面的设置 封面和富文本内容这两个内容需要特殊进行判断一下，因为在 data 中不携带这两个东西
                        if (dataObj[key]) {
                            // 有封面
